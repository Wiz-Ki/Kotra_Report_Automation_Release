...
중간에 `중지` 또는 `강제종료`한 뒤 이어서 처리하려면 GUI의 `완료 건너뛰고 재시작` 버튼을 누릅니다. 프로그램은 `logs\processing_status.xlsx`에서 `처리완료`이고, 현재 엑셀 행 내용이 완료 당시와 같으며, 저장된 PDF 파일이 실제로 존재하는 행만 건너뜁니다.
...
`processing_status.xlsx`에는 행 단위 진행 상태와 함께 `하위작업요약`, `완료작업`, `실패작업`, `남은작업`이 표시됩니다. 추천 연동의 상세 작업 기록은 프로그램 재개를 위해 `report_tasks.xlsx`에도 따로 저장됩니다.

로그 엑셀 파일을 열어둔 상태에서 프로그램이 기록을 저장해야 하면 최대 2분 동안 자동으로 다시 시도합니다. 파일을 닫으면 이어서 진행되며, 계속 잠겨 있으면 안내 오류를 표시하고 멈춥니다.
...
GUI에서 `완료 건너뛰고 재시작` 버튼을 누르면 `logs\processing_status.xlsx` 기준으로 이미 완료된 행을 건너뛰고 남은 행만 실행합니다. 완료 기록이 있더라도 엑셀 행 내용이 바뀌었거나, 저장된 PDF 파일이 삭제 또는 이동된 경우에는 다시 처리합니다.
